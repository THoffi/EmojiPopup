...
class EmojiconSpan extends DynamicDrawableSpan {
    ...
    public Drawable getDrawable() {
        if (mDrawable == null) {
            try {
                //mDrawable = mContext.getResources().getDrawable(mResourceId); // ALT
                mDrawable = AppCompatResources.getDrawable(mContext, mResourceId);
                mHeight = mSize;
                mWidth = mHeight * Objects.requireNonNull(mDrawable).getIntrinsicWidth() / mDrawable.getIntrinsicHeight();
